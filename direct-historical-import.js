...
// Configuration
const BATCH_SIZE = 10; // Process 10 funds at a time
const MONTHS_TO_IMPORT = 6; // Start with 6 months for faster initial results
  ...
  try {
    ...
    const fundsResult = await pool.query(`
      ...
      ORDER BY 
        ...
        END,
        f.id
      LIMIT 50  -- Start with 50 highest priority funds first
    `);
    ...
    for (let i = 0; i < fundsToProcess.length; i += BATCH_SIZE) {
      ...
      for (const fund of batch) {
        const result = await processFundHistoricalData(fund);
        processedCount++;
